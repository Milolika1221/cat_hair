...
class RecommendationService(IRecommendationService):
    ...
    async def get_recommendations(self, cat_id: int) -> RecommendationResult:
        ...
        try:
            ...
            analyzed_haircuts = await self._apply_rules(
                characteristics_dto_list, all_haircuts
            )
            ...
            for analysis_with_rec in analyzed_haircuts:
                for recommendation in analysis_with_rec.recommendations:
                    haircut_id = None
                    if not recommendation.confidence:
                        haircut = next(
                            (
                                h
                                for h in all_haircuts
                                if h.name == recommendation.haircut_name  # pyright: ignore[reportGeneralTypeIssues]
                            ),
                            None,
                        )
                    ...
                    await self.recommendations_repo.create(
                        cat_id=cat_id,
                        haircut_id=haircut_id,  # pyright: ignore[reportArgumentType]
                        is_no_haircut_required=recommendation.is_no_haircut_required,  # pyright: ignore[reportAttributeAccessIssue]
                        reason=recommendation.suitability_reason,  # pyright: ignore[reportAttributeAccessIssue]
    ...
    async def _apply_rules(
        ...
        for analysis_res in characteristics:
            ...
            for haircut in haircuts:
                ...
                if analysis_res.hair_length in (
                    getattr(haircut, "suitable_hair_lengths", []) or []
                ):
                    score += 1
                    reasons.append(f'Подходит для "{analysis_res.hair_length}" шерсти')

                if analysis_res.color in (
                    getattr(haircut, "suitable_colors", []) or []
                ):
                    score += 1
            ...
            results.append(
                AnalysisWithRecommendations(
                    analysis_result=analysis_res, recommendations=recommendations
                )
            )
    ...
    async def _rank_recommendations(
        analyzed_haircuts: List[AnalysisWithRecommendations],
    ...
    @staticmethod
    def _format_reason(
        match_reasons: List[str], score: float, confidence: float
    ) -> str:
        ...
        return (
            f"{reasons_text}. {confidence_text} в рекомендации (оценка: {score:.1%})."
        )
