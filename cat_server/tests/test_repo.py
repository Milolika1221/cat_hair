...
from cat_server.infrastructure.repositories import CatsRepository, CatCharacteristicsRepository
...
async def test_create_cat():
    async with AsyncSessionLocal() as session:
        ...
        characteristic = await characteristic_repo.create(cat_id=cat.id, color='black', hair_length='long', analyzed_at=datetime.now(), confidence_level=0.6)
